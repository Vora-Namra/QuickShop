...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private void insertSampleProducts(SQLiteDatabase db) {
        String[] products = {
                "INSERT INTO " + TABLE_PRODUCTS + " (" +
                        ...
                        COLUMN_PRODUCT_IMAGE_URL + ") VALUES ('Smartphone', 'High-end smartphone with advanced features', 999.99, 'https://images.pexels.com/photos/404280/pexels-photo-404280.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=750&w=1260')",
                "INSERT INTO " + TABLE_PRODUCTS + " (" +
                        COLUMN_PRODUCT_NAME + ", " +
                        COLUMN_PRODUCT_DESCRIPTION + ", " +
                        COLUMN_PRODUCT_PRICE + ", " +
    ...
    public User getUser(String username, String password) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.query(TABLE_USERS, new String[]{COLUMN_USER_ID, COLUMN_USERNAME, COLUMN_EMAIL},
    ...
    public List<CartItem> getCartItems(int userId) {
        List<CartItem> cartItems = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        String query = "SELECT c.id, p.id, p.name, p.price, c.quantity FROM " + TABLE_CART + " c " +
    ...
    public List<Order> getUserOrders(int userId) {
        List<Order> orders = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        String query = "SELECT * FROM " + TABLE_ORDERS + " WHERE " + COLUMN_ORDER_USER_ID + " = ? ORDER BY " + COLUMN_ORDER_DATE + " DESC";
        Cursor cursor = db.rawQuery(query, new String[]{String.valueOf(userId)});
